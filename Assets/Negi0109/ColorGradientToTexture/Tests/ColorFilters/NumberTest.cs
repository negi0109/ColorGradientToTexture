...
namespace Negi0109.ColorGradientToTexture.Tests.ColorFilterTests
{
    ...
    {
        [TestCase(3f)]
        public void Free(float value)
        {
            float[,] array = new float[,] { { 0f, 1f } };
            var segment = new Utils.ArraySegment2D<float>(array);
        ...
        [TestCase(0f, 1f, 0.5f)]
        public void Avg(float v0, float v1, float expected)
        {
            float[,] array = new float[,] { { v0, v1 } };
            var segment = new Utils.ArraySegment2D<float>(array);
        ...
        [TestCase(0f, 1f, 1f)]
        public void Max(float v0, float v1, float expected)
        {
            float[,] array = new float[,] { { v0, v1 } };
            var segment = new Utils.ArraySegment2D<float>(array);
        ...
        [TestCase(0f, 1f, 0f)]
        public void Min(float v0, float v1, float expected)
        {
            float[,] array = new float[,] { { v0, v1 } };
            var segment = new Utils.ArraySegment2D<float>(array);
